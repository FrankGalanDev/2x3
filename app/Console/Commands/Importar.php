...
class Importar extends Command
{
        ...
        foreach ($nuevoxml->ArticulosD->children() as $item)
            {
                ...
                $subfamilias2[]  = strip_tags($item->subfamilia2); 
                $subfamilias3[]  = strip_tags($item->subfamilia3); 
                $subfamilias4[]  = strip_tags($item->subfamilia4); 
                $eans[]  = strip_tags($item->ean); 
                $tallas[]  = strip_tags($item->talla); 
                $descripcions[]  = strip_tags($item->descripcion);
  ...
  //recorremos el array con un for para comenzar a crear el json, añadimos comillas y eliminamos las que no nos interesan
        ...
        {      
            ...
            $articulosImportado.= '"subfamilia2"'.":".'"'.$subfamilias2[$i].'"'.",";
            $articulosImportado.= '"subfamilia3"'.":".'"'.$subfamilias3[$i].'"'.",";
            $articulosImportado.= '"subfamilia4"'.":".'"'.$subfamilias4[$i].'"'.",";
            $articulosImportado.= '"ean"'.":".'"'.$eans[$i].'"'.",";
            $articulosImportado.= '"talla"'.":".'"'.$tallas[$i].'"'.",";
            $articulosImportado.= '"descripcion"'.":".'"'.preg_replace("[\"|\n|\r|\n\r|\t]","",trim($descripcions[$i])).'"'.",";
